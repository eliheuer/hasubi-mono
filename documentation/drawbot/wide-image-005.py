...
SAMPLE_TEXT_3 =  "بــــــــــــسم الله الرّحمن الرّحيم "
...
def draw_main_text():
    ...
    text("تفسير سورة", (MARGIN+(UNIT*6.2), TOP_TEXT-UNIT))
    text("الشمس", (MARGIN+(UNIT*8.95), TOP_TEXT-(UNIT*2)))

    #text("بـــــــــــــك يا علي بك يا وفي بك يا بهي", ((MARGIN-4)+MARGIN*13.55, (TOP_TEXT)-(LEADING*1)))
    fontSize(31.5)
    lineHeight(44)
    textBox(SAMPLE_TEXT_1*1, (MARGIN+(UNIT*16), MARGIN+(UNIT), MARGIN*5.2, MARGIN*3.1), align="left")
    textBox(SAMPLE_TEXT_3+SAMPLE_TEXT_4, (MARGIN+(UNIT*2), MARGIN+(UNIT), MARGIN*5, MARGIN*3.1), align="right")
    oval(MARGIN+(UNIT*13), MARGIN+(UNIT*9), UNIT*2, UNIT*2)
    oval(MARGIN+(UNIT*13.5), MARGIN+(UNIT*6), UNIT*1, UNIT*1)
